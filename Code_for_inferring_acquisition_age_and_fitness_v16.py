...
Input:
    ...
    Also requires the following files in the Data_files folder (see Data availability below):
    - UKCTOCS_non-germline_variants_calls_SNVs_indels_mCAs.csv
    - UKCTOCS_germline_variants_calls_SNV_indel_panel.csv
    - mCA_calls_with_confidence_intervals.csv
    - Clonal_structure_for_each_sample.csv
    - UKCTOCS_samples_processed_information.csv
    - DCS +/- SSCS VCF files (total depth at any panel position, used to bound the VAF at
      timepoints where a variant was not detected)

Data availability:
    Most inputs are individual-level participant data and are not distributed with this code.
    They are available under controlled access, deposited in the European Genome-phenome Archive
    (EGA) and released to approved researchers via a Data Access Committee administered by the
    corresponding authors. With these files in Data_files/, the script runs unchanged.

    file                                                  contents                                          location
    --------------------------------------------------------------------------------------------------------------
    UKCTOCS_non-germline_variants_calls_SNVs_indels_mCAs  somatic SNV / indel / mCA calls per timepoint      EGA
      .csv                                                (published, rounded, as Supplementary Tables 6-7)
    UKCTOCS_germline_variants_calls_SNV_indel_panel.csv   germline calls, annotated on the trajectory plots  EGA
    mCA_calls_with_confidence_intervals.csv               the mCA caller output with 95% confidence          EGA
                                                          intervals on cell fraction; the unrounded form of
                                                          Supplementary Table 7. Used to bound mCA
                                                          trajectories at non-significant timepoints
    UKCTOCS_samples_processed_information.csv             age at sample collection, age at AML diagnosis,    EGA
                                                          matched sample
    VCF_files/{DCS,SSCS}/*_all_positions.vcf              per-timepoint read depth at every panel position   EGA
    Clonal_structure_for_each_sample.csv                  inferred clonal structure (which variants sit in   Data_files
                                                          which clone) per sample

    The mCA calls are produced by the two mCA callers in this repository (see the
    Supplementary Fig. 26-38 notebooks); the somatic and germline call tables are produced by
    Post_processing_variant_calls.ipynb.
...
Outputs:
    ...
    7) Text file of optimiser likelihoods, inferred fitnesses and establishment times and plotted seeds
...
Usage:
Code_for_inferring_acquisition_age_and_fitness_v16.py  --sample_name --seeds
...
multiple_mutant_clone_colors = {1: {1: ['#6ba3d6'],
                                   ...
                                   5: ['#08306b', '#08519c', '#4292c6', '#9ecae1', '#deebf7'],
                                    6: ['#08306b', '#08519c', '#4292c6', '#6baed6', '#9ecae1', '#deebf7']},
                               2: {1: ['#41ab5d'],
                                  2: ['#006d2c', '#74c476'],
...
#Timepoints excluded from all analyses because the sequencing library does not represent the individual
#it is labelled with (see the post-processing notebook, where the evidence for each is recorded).  They are
#skipped when building the list of sampled ages, so that a variant absent from an excluded timepoint is not
#treated as having been looked for and not found there.
    ...
    with open('Data_files/UKCTOCS_samples_processed_information.csv') as csvfile:
        ...
        for row in readreader:
            if row_count>0:
                ...
                timepoint = row[1]
                if timepoint in timepoints_to_exclude: #excluded library: not a timepoint at which variants were assessed
                    row_count+=1
                    continue
...
#         if row['VAF (cell fraction for mCAs)'] != 'not processed':
        ...
        if gene_variant != '':
            if variant_type not in ('CNLOH', 'CN-LOH', 'GAIN', 'gain', 'LOSS', 'loss'):
                chromosome = row['chromosome']
                start = int(row['start'])
                ref = row['REF']
...
def germline_variants(person_ID, df_germline): #extract trajectories of each variant for a given sample, e.g. 'C92_002'
    ...
    for index, row in filtered_df_germline.iterrows():
        ...
        gene=row['gene']
        # VAF=float(row['VAF (cell fraction for mCAs)'])
        VAF_raw = row['VAF (cell fraction for mCAs)']
        if str(VAF_raw) == 'not processed':
            print(f"'not processed' found in germline file: sample={row['sample name']}, gene={row['gene']}, variant={row['AA_change']}")
            continue
        VAF = float(VAF_raw)
        age=row['age_sample_taken']
        try:
            ...
            print("TypeError encountered with values:")
            print("gene:", gene)
            print("variant:", variant)
            gene_variant = str(gene)+'_'+str(variant)
        exac = row['exac_all']
...
def bounded_VAF(sample_name, sample_timepoint, chromosome, position, ref, variant): #get the total read depth at a position from the all positions vcf file
    ...
    print('variant = ', variant)
    print('df depth = ', df['depth'])
    if len(df['depth'].tolist())>0:
...
def create_bounded_variant_trajectories(sample_name, sample_variants, sample_ages_lists, ages_dict_names, variant_details): #creates trajectories (with 2 timepoints) for variants with only 1 timepoint (final or first timepoint)
    ...
    for variant, timepoints in sample_variants.items():
        ...
        if sample_timing_first_detected > 0: #i.e. if the age at which it is first detected is not the first age
            ...
            print('sample ID first detected = ', sample_ID_first_detected)
            ...
            print('sample ID non detected = ', sample_ID_non_detected)
            ...
            print('ref = ', ref)
            print('variant_details = ', variant_details)
            if len(ref)>1:
                ref = ref[0]
            ...
            if variant_reads <=1: #i..e if the variant genuinely not detected
                ...
                if sample_timing_first_detected>=2: #i.e. 2 missing previous timepoints -> also had the bound for 2 timepoints before
                    ...
                    print('variant reads 2 before = ', variant_reads_two_before)
            ...
            else: #if variant reads are present (but filtered out due to error), look at the timepoint before that if it is also missing:
                if len(detected_times) == 1:
                    print('only 1 detected timepoint and penultimate timepoint has variant reads, but must have been filtered out')
                    if sample_timing_first_detected >= 2:
                        print('missing 2 previous timepoints, so check previous timepoint')
                        two_before_non_detected_age = sample_timings[sample_timing_first_detected-2] #i.e. two timepoints/ages before 1st detected
                        sample_ID_two_non_detected = ages_dict_names[two_before_non_detected_age] #e.g. C92_002_s7
                        non_detected_bounded_VAF_two_before, variant_reads_VAF_two_before = bounded_VAF(sample_name, sample_ID_two_non_detected, chromosome, position, ref, variant)
                        variant_reads_two_before = variant_reads_VAF_two_before[0]
                        ...
                        if variant_reads_two_before <=1:
                            print('2 timepoints before genuinely missing')
                            bounded_sample_variants[variant] = {two_before_non_detected_age: float(non_detected_bounded_VAF_two_before)}
        ...
        if last_detected_timepoint < sample_timings[-1]: #i.e. when younger than final timepoint sample
            ...
            else: #if variant reads are present (but filtered out due to error), look at the next timepoint to see if it is also missing:
                if len(detected_times) == 1:
                    print('only 1 detected timepoint and next timepoint has variant reads, but must have been filtered out')

                    if last_detected_timepoint < sample_timings[-2]: #i.e. more than 2 timepoints missing at the end:
                        second_non_detected_age = sample_timings[sample_timing_last_detected+2] #i.e. the 2nd timepoint/age when first not detected (i.e. timepoint after last detected)
                        sample_ID_second_non_detected = ages_dict_names[second_non_detected_age] #e.g. C92_002_s8
                        second_non_detected_bounded_VAF, variant_reads_VAF_second_non_detected = bounded_VAF(sample_name, sample_ID_second_non_detected, chromosome, position, ref, variant)
                        variant_reads_two_after = variant_reads_VAF_two_after[0]
                        print('variant reads 2 after = ', variant_reads_two_after)

                        if variant_reads_two_after <=1:
                            print('2 timepoints after genuinely missing')
                            bounded_sample_variants[variant] = {second_non_detected_age: float(second_non_detected_bounded_VAF)}
...
def create_bounded_mCA_trajectories(sample_name, sample_variants, df_mca_ci):
    """
    For mCA variants with non-significant phased calls, add the 95% upper CI
    as a bounded observation — mirroring how 1/depth bounds work for SNVs.
    Returns a dict in the same format as bounded_sample_variants.
    """
    bounded_mca = {}

    # Filter CI file to this patient
    patient_ci = df_mca_ci[df_mca_ci['sample name'].str.startswith(sample_name)]

    # Only non-significant phased rows
    non_sig = patient_ci[
        (patient_ci['significant (phased caller)'] == False) &
        (patient_ci['mCA caller'] == 'phased')
    ]

    for _, row in non_sig.iterrows():
        chrom = row['chromosome']
        event = row['mCA type'].upper().replace('-', '')  # e.g. 'CN-LOH' -> 'CNLOH'
        variant_key = chrom + '_' + event   # e.g. 'chr7_LOSS', 'chr19_CNLOH'

        # Match to the key used in sample_variants (which uses chromosome with arm suffix)
        # Find matching key in sample_variants
        matching_key = None
        for sv_key in sample_variants.keys():
            # sv_key e.g. 'chr7q_LOSS', 'chr19q_CNLOH'
            sv_chrom = sv_key.split('_')[0]
            sv_event = '_'.join(sv_key.split('_')[1:])
            if sv_chrom.startswith(chrom) and sv_event == event:
                matching_key = sv_key
                break
        ...
        age = row['age sample taken']
        upper_cf = row['95% upper cell fraction (phased caller)']

        if pd.isna(upper_cf):
            continue

        if matching_key not in bounded_mca:
            # Include all observed (significant) timepoints too
            bounded_mca[matching_key] = {k: v for k, v in sample_variants[matching_key].items()}

        bounded_mca[matching_key][age] = float(upper_cf)

    # Sort by age
    for variant in bounded_mca:
        bounded_mca[variant] = dict(sorted(bounded_mca[variant].items()))
...
#                     ax1.annotate(clone, (establishment, fitness))
                ...
                if clone in plotted_seeds.keys():
                    plotted_seeds[clone].append((establishment, fitness))
                else:
                    plotted_seeds[clone]=[(establishment, fitness)]
#                 print()
    ...
    ax1.set_xlim(((min(establishment_times))-5), (max((max(establishment_times)+5), (diagnosis_age+5))))
    ...
    return final_seed_likelihoods, seed_with_best_likelihood, best_likelihood, plotted_seeds
        ...
        if mutation == 'JAK2_p.V617F':
            ...
            if CNLOH_present > 0:
                for age, VAF in trajectory.items():
                    chr9p_cell_fraction = sample_variants['chr9p_CNLOH'].get(age, None)
                    if chr9p_cell_fraction is not None:
                        JAK2_cell_fraction = ((VAF - chr9p_cell_fraction) * 2) + chr9p_cell_fraction
                    else:
                        JAK2_cell_fraction = VAF * 2  # mCA not significant at this timepoint
            ...
            if CNLOH_present > 0:
                for age, VAF in trajectory.items():
                    chr4q_cell_fraction = sample_variants['chr4q_CNLOH'].get(age, None)
                    if chr4q_cell_fraction is not None:
                        TET2_cell_fraction = ((VAF - chr4q_cell_fraction) * 2) + chr4q_cell_fraction
                    else:
                        TET2_cell_fraction = VAF * 2
...
def plot_trajectories_cell_fractions(sample_name, optimised_clones, T, dt, sample_variants, bounded_sample_variants, sample_ages, sample_ages_lists, diagnosis_age, upper_freq, seeds, multiple_mutant_clone_colors, df_germline, plot_bounded_or_not, variant_depths_by_age):
    ...
    for variant, new_traj in new_mut_traj_ordered_cell_fractions.items():
        ...
        if variant not in sample_variants_cell_fractions:
            print(f"WARNING: '{variant}' not found in sample_variants_cell_fractions")
            print(f"Available keys: {list(sample_variants_cell_fractions.keys())}")
...
#         print('clone = ', clone)
        ...
        if clone == 'stem cells':
            ...
            if clone_parent == 'stem cells':
                ...
                parent_descendents = clone_mutation_number.get(1,0) #the number of single mutant clones in the phylogeny
            else:
...
def main():
    ...
    df_mca_ci = pd.read_csv("Data_files/mCA_calls_with_confidence_intervals.csv")
    ...
    # Merge bounded mCAs into bounded_sample_variants
    bounded_mca = create_bounded_mCA_trajectories(sample_name, sample_variants, df_mca_ci)
    print('bounded_mca found:', list(bounded_mca.keys()))
    for variant, timepoints in bounded_mca.items():
        bounded_sample_variants[variant] = timepoints
        all_trajectories_with_bounded[variant] = timepoints
    ...
    final_seed_likelihoods, seed_with_best_likelihood, best_likelihood, plotted_seeds = plot_seed_results(optimised_seeds, sample_name, seeds, clone_colors, diagnosis_age)
    ...
    pdfs = ['Data_files/'+sample_name+'_optimiser_results_'+str(seeds)+'_seeds_v'+str(version)+'.pdf',
    ...
    optimiser_results_file.write(dt_string+'\t'+sample_name+'\t'+str(seeds)+'\t'+str(final_seed_likelihoods)+'\t'+str(seed_with_best_likelihood)+'\t'+str(best_likelihood)+'\t'+str(fitnesses)+'\t'+str(tau_times)+'\t'+str(establishment_times)+'\t'+str(variant_colors)+'\t'+str(clone_colors)+'\t'+version+'\t'+str(plotted_seeds)+'\n')
