...
| data | where |
|---|---|
| <ul><li>raw sequencing reads</li><li>the per-timepoint variant call files at every stage, as listed in [Variant call file outputs](#variant-call-file-outputs)</li><li>germline SNV and indel calls</li><li>the combined somatic SNV, indel and mCA call table (the exact-age version of the rounded table provided here)</li><li>longitudinal mCA calls</li><li>per-sample BAF and LRR files</li><li>participant sample and clinical annotation files with exact ages</li></ul> | European Genome-phenome Archive, study accession [EGAS50000002168](https://ega-archive.org/studies/EGAS50000002168) — datasets EGAX50000049261 (SNV/indel panel) and EGAX50000049262 (mCA/chromosomal rearrangement panel). Controlled access, via a Data Access Committee administered by the corresponding authors |
| The final filtered somatic variant calls (hg19), as per-sample VCF records plus the <em>FLT3</em>-ITD in the Pindel caller's own output. No ages or clinical data; the same calls with annotation and rounded ages are Supplementary Table 6 | Zenodo, DOI [10.5281/zenodo.22262497](https://doi.org/10.5281/zenodo.22262497) |
| hg19 reference genome (<code>Homo_sapiens_assembly19.fasta</code>, Broad b37/GRCh37) with its index files. The Zenodo archive holds the FASTA, <code>.fai</code>, <code>.dict</code> and BWA index (4 GB compressed, ~8 GB unpacked); the Broad copy is the FASTA alone (~3 GB) and needs indexing | Zenodo, DOI [10.5281/zenodo.22846473](https://doi.org/10.5281/zenodo.22846473)<br>or the [Broad's public bucket](https://storage.googleapis.com/gcp-public-data--broad-references/hg19/v0/Homo_sapiens_assembly19.fasta) |
| UK Biobank data (Fig. 5f) | UK Biobank, obtained under Application 58387 and available through the [Access Management System](https://www.ukbiobank.ac.uk/enable-your-research/apply-for-access) |
