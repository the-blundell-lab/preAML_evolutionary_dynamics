"""
figure5d_50yr_pipeline.py -- supplementary version of Figure 5d: the SAME virtual-AML variant
trajectories as the main panel, but with the pre-diagnosis window extended from 15 to 50 years
(x-axis -50 .. 0 yr before AML). Same model, seeds, cases, colours (blue 1 / green 2 / orange 3 /
red 4), faint 50-case ensemble and bold focal trajectories. The focal trajectories are selected on
the SAME -15..-1 yr behaviour as the main panel, so they are identical clones shown over a longer
history. Sampled at 2000-4000x sequencing depth. Model = sim_fast.py; default s=0.16, r=2.5, p=3.

Usage:  python figure5d_50yr_pipeline.py --sims 40000 --workers 4 --out Figure5d_trajectories_50yr
"""
import os
...
LEFT = -50                                        # extended window: LEFT .. 0 yr before diagnosis
YEARS = list(range(LEFT, 1))                       # -50 .. 0
FOCAL_YEARS = list(range(-15, 0))                  # focal behaviour judged on the main-panel window
...
# ---------- behaviour detectors (judged on the -15..-1 sub-window, exactly as the main panel) ----------
def already_swept(y):
...
def make_figure(cases, out, s1, r, p):
    ...
    for variants in cases:
        for v in variants:
            d = dict(zip(v['yr'], v['cf']))
            w = [d[y] for y in FOCAL_YEARS if y in d]         # the -15..-1 sub-window
            if len(w) < len(FOCAL_YEARS):
                continue
    ...
    for variants in sample:
        for v in variants:
            if max(v['cf']) < 1 or id(v) in focal_ids: continue
    ...
    ax.set_xticks(list(range(LEFT, 1, 10))); ax.set_yticks(range(0, 101, 20))
...
def main():
    ...
    ap.add_argument('--out', type=str, default='Figure5d_trajectories_50yr')
    ...
    with Pool(a.workers) as pool:
        parts = pool.map(worker, [(per, 700000 + i) for i in range(a.workers)])   # SAME seeds as main 5d
